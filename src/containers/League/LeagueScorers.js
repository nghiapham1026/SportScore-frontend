...
const LeagueScorers = ({ leagueId, season }) => {
  ...
  if (!topScorers.length) {
    return (
      <p className={styles.leagueScorersNoData}>
            ...
            <tr key={scorer.player.id}>
              <td className={styles.leagueScorersPlayerRow}>
                <div className={styles.leagueScorersPlayerInfo}>
                  <span className={styles.leagueScorersPlayerRank}>
                    {index + 1}.
                  </span>
                  <Link to={`/players/${scorer.player.id}`}>
                    <img
                      ...
                      className={styles.leagueScorersPlayerPhoto}
                    />
                    <span className={styles.leagueScorersPlayerName}>
                      {scorer.player.name}
                    </span>
                  </Link>
                  <Link to={`/team/${scorer.statistics[0].team.id}`}>
                    <img
                      ...
                      className={styles.leagueScorersTeamLogo}
                    />
                    <span>{scorer.statistics[0].team.name}</span>
                  </Link>
                </div>
              </td>
              <td className={styles.leagueScorersGoals}>
